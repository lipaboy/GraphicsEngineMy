...
void DX11GraphicsEngine::SetViewport(Rect rect)
{
	...
	D3D11_VIEWPORT vp;
	{
		vp.TopLeftX	= static_cast<FLOAT>( m_viewportRect.x * m_width  );
		vp.TopLeftY	= static_cast<FLOAT>( m_viewportRect.y * m_height );
		vp.Width	= static_cast<FLOAT>( m_viewportRect.w * m_width  );
		vp.Height	= static_cast<FLOAT>( m_viewportRect.h * m_height );
		vp.MinDepth	= 0.0f;
		vp.MaxDepth	= 1.0f;
	}
...
HRESULT DX11GraphicsEngine::InitDevice()
{
	...
	Application::Instance().SetContext( new DX11GraphicsEngineContext(m_pDevice, m_pImmediateContext, this) );
